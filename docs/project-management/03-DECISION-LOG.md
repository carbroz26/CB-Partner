...
The documentation/routing portion is implemented. The actual Wrapper files remain blocked until a compatible Gradle 9.7.1 executable is provisioned, because the official Wrapper must be generated by the Gradle wrapper task and its JAR must not be manually fabricated.
