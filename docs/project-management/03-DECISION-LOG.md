...
The repository must use the official Gradle Wrapper for normal builds. BASE-ARCH-014-T01 targets Gradle 9.5.0 because the repository currently uses Android Gradle Plugin 9.3.0, whose documented minimum/default compatible Gradle version is 9.5.0.
...
The documentation/routing portion is implemented. The actual Wrapper files remain blocked until a compatible Gradle 9.5.0 executable is provisioned, because the official Wrapper must be generated by the Gradle wrapper task and its JAR must not be manually fabricated.
